...
| **LLM 内部 attention**（saliency proxy） | FastV, PyramidDrop | HiDivDrop (DTop-K, 训练式隐式 A2) |
| **多样性 / 覆盖** | DivPrune | SCOPE, HoloV |
| **跨模态文本**（显式 text-guided） | SparseVLM (LLM attn) | CDPruner, FSR, Nuwa, VisionTrim |
| **重要性 + 多样性** | — | IDPruner (MMR) |
| **时序一致性** | HiDivDrop (ILVAS, 层选择辅助) | VScan (Global+Local) |
...
- $\tilde{r}_{i}$：第 $i$ 个视觉 token 与指令 embedding 的 cosine similarity 经 min-max 归一化到 [0, 1]（相关性）
...
$$v^* = \arg\max_{v_i \notin \mathcal{S}} \Bigl[\lambda \cdot \text{Imp}(v_i) - (1-\lambda) \cdot \underbrace{\max_{v_j \in \mathcal{S}} \cos(v_i,v_j)}_{m_i}\Bigr]$$
...
| **HiDivDrop** | ICLR 2026 | LLM last-text-token attn + DTop-K（训练） | ✅（隐式，last text token attn） | 层间一致性（ILVAS 选层） | LLM 内（多层） | ❌ |
...
- **代表方法**：VisionZip、SCOPE（saliency 项）、HoloV（`𝒜ᶜ` 项）、VScan（Global/Local CLS）、VisionTrim（`Sᵍ` 项）、FSR（`sᵢ`）、Nuwa（`α_cls` × `‖kᵢ‖₂`）、IDPruner（VisionSelector）
...
- **代表方法**：
  - *LLM 内部 attention 路线*：FastV（LLM L2 全 token → visual received attn 均值）、PyramidDrop（multi-stage last-instr Q·K → visual attn）、SparseVLM（text rater 筛选后 text → visual attn）、VScan（LLM 中间层 last-instr attn，LLaVA k=16 / Qwen-7B k=14）、Nuwa（Stage 2 text-visual cos sim）、HiDivDrop（filtering layer 处 last-text-token → visual attn，multi-round + DTop-K 可微选择，训练式隐式 A2）
...
| **CDPruner** | — | 计算每个 visual token 与指令 embedding 的 cosine similarity，经 **min-max 归一化**映射到 [0, 1] 作为 task-relevance 权重 $\tilde{r}\_i$。兼容两种 embedding 来源：CLIP text encoder（有配对 text encoder 的架构，如 LLaVA）或 LLM instruction token 平均 embedding（无独立 text encoder 时，如 Qwen2.5-VL） | Conditional DPP：条件核 $\tilde{L}\_{ij} = \tilde{r}\_i \cdot \cos(v\_i,v\_j) \cdot \tilde{r}\_j$，使 $\log\det(\tilde{L}\_S) = \sum\_{i \in S} \log \tilde{r}\_i^2 + \log\det(L\_S)$，将 relevance 与 diversity 天然分解到行列式中联合优化。MAP inference 经贪心 + Cholesky 增量分解实现，$O(nm^2)$，实测额外延迟 <10ms | — |
...
| **VisionTrim** | DVTS 双信号**逆方差加权**融合：全局信号（ViT 倒数第 2 层 CLS attention 多头均值，经 softmax 归一化）与局部信号 LTAM（k×k 窗口内以 dual-kernel — 特征空间高斯 + 加权空间位置高斯 — 计算每个 token 与邻域的亲和度均值，转为概率分布）线性融合，权重自动按方差分配：哪个信号方差越大（越不稳定），其权重越小，自动倾向更可靠的信号源 | TGVC：以 CLIP text encoder 编码用户指令，计算 text→visual **scaled dot-product attention** 后按 text token 维度平均，得到 per-token 文本相关性分数，选 top-R 为聚类中心驱动后续 merge | TGVC merge：top-R 个文本相关度最高的 remaining token 为聚类中心；剩余 token 通过 **text-mediated assignment**（token→text attention × text→center attention 的乘积）分配到最匹配中心，加权平均 + 残差连接生成 R 个 complement token | — |
| **HoloV** | $\mathcal{A}\_i^c$：ViT CLS attention 多头均值，衡量每个 token 在 crop $c$ 中的全局语义显著性。作为 holistic score 的基础分量参与后续融合 | — | $\mathcal{V}\_i^c$：token $i$ 与 crop 内所有其他 token 的 cosine similarity 的**方差**（高方差表明该 token 与不同邻居的相似度变化大——连接模式多样，处于语义边界位置）。Holistic score $\mathcal{H}\_i^c = \gamma\_c \mathcal{V}\_i^c + \mathcal{A}\_i^c$，其中 $\gamma\_c = \mathbb{E}[\lVert\mathcal{A}^c\rVert] / \mathbb{E}[\lVert\mathcal{V}^c\rVert]$ 自适应缩放使两项量级一致。该设计为 per-token 局部唯一性度量，非 MMDP/DPP/Submodular 式集合级目标。另有 Fast VCR 机制在模型输出高不确定性时动态补回被剪 token | Crop-wise adaptive allocation：$w\_c \propto \text{avg}(\mathcal{H}^c)$，每个 crop 按平均 holistic score 分配 token 配额 $q\_c$，并设上下限防止单一 crop 垄断所有额度，从几何上保证多区域覆盖 |
| **HiDivDrop** | — | DTop-K（可微 token 选择）：重要性分数 $c\_i$ 来自 LLM 在 filtering layer 处 **last text token 对各 visual token 的 attention**（multi-round scheme：重复 $n$ 轮取最终排名，默认方案；消融实验亦测试了 all-token 及 L2-norm 变体）。对 $c\_i$ 做归一化排名 $c'\_i \in [0,1]$，再施加软掩码 $\text{Mask}\_i = \sigma(\lambda(c'\_i - a))$，其中 $a$ 为可学习剪枝阈值。前向传播使用 hard threshold，反向传播以 sigmoid 梯度近似实现端到端训练。⚠️ 非 training-free。属隐式 A2（同 FastV/PyramidDrop，使用 LLM attention 作为 saliency proxy，信号依赖文本输入）。**辅助层选择机制（非 token indicator）**：$\text{ILVAS}(l) = \text{sim}(\tilde{A}^{(l)}, \tilde{A}^{(l+n)})$ 评估各层 attention 排名的跨层稳定性，选 ILVAS 曲线**局部最大值**作为剪枝层位（$\mathcal{F} = \{10, 14, 16, 18\}$），决定**在哪层剪枝**而非剪哪些 token | —（方法名称含 "Div" 但多样性体现于三段式架构设计——Late Injection 跳过浅层 / Concave Pyramid 中间层渐进 / Early Exit 深层停止——而非 indicator 层面的显式 diversity 目标函数） | — |
| **VScan** | 双尺度 union：Global Scan 取 ViT **output layer**（如 LLaVA-1.5 的倒数第 2 层）的 [CLS] attention 多头均值 $\bar{S}\_{[CLS]}^i = \frac{1}{H}\sum\_h S\_{[CLS]}^{h,i}$ 作为全局语义显著性分数（Eq. 1-2）；Local Scan 取 ViT **浅层 $l$**（LLaVA: $l{=}6$，Qwen-2.5-VL: $l{=}8$）的 [CLS] attention，在非重叠窗口内按 Eq. 3 选局部重要 token。默认 $\|\mathbf{x}\_V^g\| = \|\mathbf{x}\_V^l\|$（各占保留总量一半），取 union $\mathbf{x}\_V^{selected} = \mathbf{x}\_V^g \cup \mathbf{x}\_V^l$，总保留率 $R\_1\%$。⚠️ 对无 [CLS] token 的架构（如 Qwen-2.5-VL），改用 **self-attention**（各 token 从其他所有 visual token 收到的平均 attention）替代 [CLS] attention | LLM 中间层 $k$（LLaVA: $k{=}16$，Qwen-2.5-VL-7B: $k{=}14$，3B/32B 取各自精确中间层）：计算 last instruction token $x\_{instr}$ 与各 visual token 的 scaled dot-product attention $S\_{text}^h = \text{Softmax}((W\_Q^h x\_{instr})^T W\_K^h \mathbf{x}\_V / \sqrt{D/H})$（Eq. 5），多头取平均后保留 top $R\_2\%$（默认 $R\_2{=}33.3\%$）。选中间层而非浅层（Table 1: $k{=}16$ 优于 $k{=}2$ 约 1-2 pts），因浅层 attention 存在位置偏差（Section 3 Study 1）。通过 auxiliary vanilla attention pass 在 FlashAttention 之外单独计算 attention map，保持 FA 兼容性 | Token Merging（**仅 Stage 1 ViT 侧**，Stage 2 pruning 后不做 merge）：在 Global+Local Scan 选定 $\mathbf{x}\_V^{selected}$ 后，每个 unselected token $x\_V^u$ 按 cosine similarity 分配到最相似的 selected token $x\_V^s$，各组做 average pooling $\mathbf{x}\_V^{s,merged} = (x\_V^s + \sum\_{u \in \mathcal{U}^s} x\_V^u) / (1 + \lvert\mathcal{U}^s\rvert)$（Eq. 4），输出 token 数 = $\|\mathbf{x}\_V^{selected}\|$（不变）。保留了被丢弃 token 的信息。属信息保留机制，非显式 diversity 目标 | Local Scan 将 token grid 划分为非重叠窗口，**均匀分配 token 预算**到各窗口，每个窗口内按浅层 [CLS] attention 独立选取高分 token（Eq. 3 中 $\tau\_w$ 为窗口级软阈值），从几何上保证图像各空间区域至少保有代表 token，避免 Global Scan 的 output-layer [CLS] attention 导致选择集中于单一显著区域 |
| **Nuwa** | $S(t\_i) = \alpha\_{\text{cls},i} \times \lVert \mathbf{k}\_i \rVert\_2$：将 ViT CLS attention 与 key vector 的 L2-norm 相乘——前者衡量全局重要性，后者反映信息容量。高 $\lVert k \rVert$ 的 **Pillar Token**（ViT 的 register token）被识别并保护，不参与后续聚合 | Stage 2（LLM 中间层）：在多模态对齐完成后，以 text token embeddings 的 **average-pooling** 构造 query $\bar{q}$，计算 $R\_i = \cos(\text{proj}(v\_i'), \bar{q})$（$\text{proj}(\cdot)$ 为多模态映射层），保留 top-$K\_{\text{final}}$ 个 cosine similarity 最高的 token | 聚合权重 $W\_{ij} = \text{ReLU}(\cos(v\_i, v\_j)) \times P\_{ij}$，其中 $P\_{ij} = \max(0,\, 1 - d(p\_i,p\_j)/d\_{\text{thresh}})$ 为**线性截断距离衰减**（非高斯核），超过阈值 $d\_{\text{thresh}}$（默认 26% 最大距离）归零。双约束确保只有语义相近且空间相邻的 token 才被合并。Pillar Token（top 25% L2-norm）不参与聚合（$\delta\_{ij}$），仅 Collector Token 执行加权聚合后行归一化 $V\_B' = \hat{W}V$。属空间感知的信息保留机制 | M×M grid 分区：将 token grid 划分为 $M \times M$ 个非重叠子区域，每区域内独立选取 top-n benchmark token（按 salience score $S(t\_i)$ 排序），从几何上保证空间均匀覆盖。该设计针对 visual grounding / REC 等需要全局空间参考系的任务（论文在 RefCOCO 系列上验证），实现了比 RPME 更精确的空间完整性保持 |
| **FSR** | $s\_i$：ViT CLS attention 衡量每个 token 的视觉显著性，与 $r\_i$ 共同以乘法幂律融合构成 Focus score $\phi\_i$ | $r\_i$：CLIP text encoder 编码指令后与各 visual token 计算 cosine similarity 得到指令相关性分数。Focus score 融合为 $\phi\_i = \hat{r}\_i^{\alpha} \cdot \hat{s}\_i^{\beta}$（$\hat{\cdot}$ 为归一化，$\alpha, \beta$ 为**指数**，默认 $\alpha{=}3, \beta{=}1$），**乘法幂律**使任一信号接近零即可"否决"该 token。并以动态 budget $K\_F = \min\{k : \sum\_{j=1}^k \phi\_{\pi(j)} \geq \rho Z\}$（默认 $\rho{=}0.9$）自适应决定保留 token 数——图像语义越简单，Focus token 越少 | CCS（Conditional Context Sampling）：≈ Farthest Point Sampling 变体，每步选与 Focus + 已选 Scan token 的综合相似度**最低**的 token 以最大化信息增量，具理论覆盖界保证（2-近似）。Refine 阶段：剩余 token 按 similarity 分配到最近 Scan center，但仅选 **top-M**（$M = \kappa |S|$，默认 $\kappa{=}1$）个相似度最高的参与加权聚合，其余直接丢弃（防止 over-smoothing）；仅修改 Scan token，保护 Focus token 高保真度 | — |
| **IDPruner** | $\text{Imp}(v\_i)$：由 VisionSelector 模块以 DiffTopK 机制生成 + min-max 归一化。VisionSelector 被论文归为 importance-based 方法（据分类推断仅以 visual feature 为输入、不使用文本信息，但原文未显式声明），属**学习型 A1**（⚠️ 非 training-free，需端到端训练） | — | MMR：首步 $v^*\_1 = \arg\max\_{v\_i \in \mathcal{V}} \text{Imp}(v\_i)$（纯重要性初始化，$\mathcal{S}$ 为空无法计算相似度惩罚）；后续步 $v^* = \arg\max\_{v\_i \notin \mathcal{S}}[\lambda \cdot \text{Imp}(v\_i) - (1{-}\lambda) \cdot \max\_{v\_j \in \mathcal{S}} \cos(v\_i, v\_j)]$，以**减法**显式平衡 importance 与 diversity（对比 SCOPE 的乘法融合），$\lambda \in \{0.5,\, 0.7\}$ 并列最优（均 95.56%），默认取 $\lambda{=}0.5$。$O(KN)$ 复杂度，维护 max-similarity 向量逐步增量更新。无全局近似保证（vs DPP/Submodular 的 $(1{-}1/e)$） | — |
...
- **第一代（隐式 A2）**：FastV（全 token received attn 均值，text 信号被 img-to-img attn 稀释）、PyramidDrop（last-instr Q·K 点积，text 信号更集中）、HiDivDrop（filtering layer 处 last-text-token multi-round attn + DTop-K 可微选择，训练式）——使用 LLM attention 作为**显著性代理**，设计目标是效率而非文本语义对齐，text 信号是副产品
...
- **LLM 内部 attention 路线**（SparseVLM、VScan 中间层、Nuwa Stage 2）：更通用，但需要额外 forward pass 或访问中间层激活
