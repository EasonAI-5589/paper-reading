...
- **代表方法**：
  - *LLM 内部 attention 路线*：FastV（LLM L2 全 token → visual received attn 均值）、PyramidDrop（multi-stage last-instr Q·K → visual attn）、SparseVLM（text rater 筛选后 text → visual attn）、VScan（LLM L16 last-instr attn）、Nuwa（Stage 2 text-visual cos sim）、HiDivDrop（filtering layer 处 last-text-token → visual attn，multi-round + DTop-K 可微选择，训练式隐式 A2）
...
| **VScan** | 双尺度 union：Global Scan 取 ViT **output layer** 的 CLS attention 捕捉高层语义显著性，Local Scan 取 ViT **layer 6** 的 CLS attention 保留低层细节信息，两组各选 $R\_1/2$ 个 token 后取 union 实现多粒度互补 | LLM **中间层**（LLaVA: k=16, Qwen-2.5-VL: k=14）last-instruction token 对 visual tokens 的 attention 多头均值，保留 top $R\_2\%$。关键设计选择：选中间层而非浅层（实验验证 k=16 远优于 k=2），因浅层 attention 存在位置偏差。以 auxiliary vanilla attention pass 提取 attention map，与 FlashAttention 兼容 | Token Merging：每个 **unselected** token 按 cosine similarity 分配到最相似的 **selected** token，组内（selected token + 所有分配到它的 unselected tokens）做 average pooling，输出 token 数 = |selected|（不变）。保留了被丢弃 token 的信息。属信息保留机制，非显式 diversity 目标 | Local Scan 将 token grid 划分为非重叠窗口，每个窗口内独立选取 top-k，从几何上保证图像各空间区域至少保有代表 token，避免全局 CLS attention 导致选择集中于单一区域 |
