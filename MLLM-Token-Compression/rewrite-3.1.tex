...
Tethered to a single-indicator paradigm, existing visual token pruning methods rest on an assumption that has not been systematically examined: a single importance criterion is sufficient to distinguish essential tokens from redundant ones.
Our investigation reveals that the indicators employed by existing methods naturally fall into two distinct families: \textit{saliency}, which quantifies individual token importance (VisionZip~\cite{visionzip}, CDPruner~\cite{cdpruner}), and \textit{diversity}, which promotes collective representativeness of the retained set (DivPrune~\cite{divprune}, SCOPE~\cite{scope}).

As illustrated in Fig.~\ref{fig:indicator_vis}, at the same compression ratio, these two indicator families produce notably different token selections.
VisionZip, guided by ViT CLS attention, primarily retains foreground tokens of high semantic saliency---a selection that is independent of the user's query.
...
SCOPE strikes a balance between the two, coupling CLS-based saliency with a submodular coverage term that curbs token clustering within narrow semantic neighborhoods.

To be specific, methods sharing the same indicator family show moderate agreement---for instance, VisionZip and SCOPE, both grounded in CLS-based saliency, achieve an overlap of 0.64 at $K{=}128$.
Cross-family overlap, however, drops sharply: at $K{=}32$, it falls to only 0.19--0.27.
Even under the most generous retention budget ($K{=}128$, retaining 22\% of all visual tokens), the average pairwise overlap remains below 0.45---implying that \emph{more than half of the tokens deemed ``most important'' by one indicator family are considered unimportant by the other}.
