...
> 💡 **核心证据：Tab. 1**（12 benchmark × 15 baselines 全量对比，base model = Qwen2.5-VL-7B）
...
> 💡 **核心证据：Tab. 5（MuirBench 子任务）+ Tab. 6（LogicVista 子任务）**，揭示短期/长期记忆各自的分工：
> - **短期记忆擅长**：counting (+7.0%), visual retrieval (+9.4%), grounding (+13.1%)（需要回看图片细节）
> - **长期记忆擅长**：inductive (+5.7%), deductive (+7.1%) reasoning（需要语义知识）
> - 两者互补 → 合体效果远超单独使用
...
> 💡 **核心证据：Fig. 3（柱状图）+ Tab. 7（具体数字）**，实验设置：只在 Visual CoT + Mulberry 2 个数据集上训练，测 4 个 unseen benchmark。
> - VisMem 在所有 4 个 benchmark 上领先第二名 2.7~6.8%
> - 最高跨域提升 +20.2%（MV-Math），最低 +6.9%（MMVet）
> - 与 full training 的差距仅 ~2%，说明 latent vision memory 学到的是**通用视觉认知能力**，而非任务特定 trick
...
> 💡 **核心证据：Fig. 4（曲线图）+ Tab. 8（具体数字）**，实验设置：4 阶段顺序训练，每阶段叠加新数据，在 MMVet 上测试。
> - **SFT**：性能降 >10%（最严重的灾难性遗忘）
> - **VLM-R1/Vision-R1**：Stage 0 提升 ~11%，到 Stage 4 几乎全部丧失（<0.5% 留存）
> - **VisMem**：Stage 0→3 保持 72.1%，是所有方法中遗忘最少的，Stage 1 和 3 甚至继续提升
>
> **为什么 VisMem 抗遗忘**：记忆存在 LoRA adapter 中，不直接改动 VLM 核心参数。训练新任务时，backbone 的原始知识不被梯度更新覆盖。
...
> 💡 **核心证据：Tab. 2 + Fig. 11**，测试 9 个 base model（Qwen2.5-VL-3B/32B、LLaVA-OV-4B/8B、InternVL-3.5-4B/8B/14B/38B，3B~38B）。
> - 全部获得显著提升（这是唯一用多模型的实验，其余主实验均用 Qwen2.5-VL-7B）
> - 小模型提升更大（Qwen2.5-VL-3B: 平均 +12.3%），大模型也有明显提升（InternVL-3.5-38B: 平均 +7.0%）
> - 证明 VisMem 是 **model-agnostic** 的架构设计
...
> 💡 **核心证据：Fig. 5（调用频率 + 位置分布）+ Fig. 9（各任务对比）+ Tab. 5, 6（子任务分工）**
> - **调用频率随序列位置递减**（Fig. 5）：开头调用多（需要锚定视觉信息），后面调用少（已积累足够上下文）
> - **任务自适应**（Fig. 9）：MuirBench（多图理解）→ 短期记忆调用更多；MV-Math（数学推理）→ 长期记忆调用更多
> - 说明模型通过 Stage II 的 RL 训练确实学会了**按需调用**，而不是机械地每步都插入
...
> 💡 **核心证据：Fig. 6（效率-性能气泡图）+ Tab. 12（具体延迟数字）**
> - VisMem 在右上角（高性能 + 适中延迟）；Image-level 方法在右下角（高延迟 + 中等性能）
> - VisMem 延迟增加仅 8.2%~43.8%（相对于 vanilla），远低于 image-level 方法的 2x+
> - 结论：latent token 额外开销远比操作像素空间便宜
...
> 💡 **核心证据：Tab. 3（主消融：记忆组件 + 调用策略）+ Tab. 9（调用效率）+ Tab. 10, 11 + Fig. 10（超参敏感性）**
>
> **Tab. 3 关键发现**（反直觉！）：
...
>
> - **100% 调用反而不如 25%**：过度记忆干扰正常推理（noise > signal），按需调用才是关键
> - **Short only vs Long only**：MuirBench（理解）→ Short 好；MV-Math（推理）→ Long 好，印证分工
> - **Complete 远超两者单独**：短期 + 长期记忆确实互补，缺一不可
