...
>
> **🔍 Table 4 三大消融结论一句话总结**：
> - **框架组件**：Visual Sketch 最关键，去掉后性能腰斩（连接高层推理与底层控制的根本桥梁）
> - **Sketch 元素**：Keypoints 最重要，去掉后下降最大（精确坐标 grounding 是核心，不是 Arrows）
> - **训练阶段**：Stage 3 不可缺少，去掉后成功率直接变为 0%（没有 Stage 3 = 模型不知道怎么把 Sketch 变成动作）
