...
> 🔍 **第二层绿色框：Hallucination-Aware Policy Optimization 完整流程**
>
> 图示从左到右分为三个步骤：
>
> **Step 1 — Keyframe-Initialized Rollouts（KIR）**
>
> - Policy 在 world model 中生成多组（Group 1 … Group n）imagined trajectories
> - **关键**：并非所有 rollout 都从 episode 初始帧 $o_0$ 开始，而是一部分从「任务关键中间帧 / failure state」附近的 keyframe $o_k$ 初始化
> - 图中 ⏸ 标记 = 跳过前期帧（skip early frames）——跳过 world model 误差积累最严重的前段，直接从离关键交互区最近的帧开始
> - 结果：每组轨迹是一段从不同起始帧展开的视频帧序列（图中灰度帧网格）
>
> **Step 2 — Calculate Reward（含 Masking）**
>
> - 对每条轨迹的每一步，reward classifier $\hat{R}_\psi$ 输出 binary reward（成功✓ / 失败✗）
> - 图中彩色矩阵：绿色✓ = 成功步骤，红色✗ = 失败步骤
> - 🔳 标记 = **mask post-success steps**：第一次出现✓后，后续所有步骤的 reward 被屏蔽
>   - 动机：成功发生后 world model 继续生成的帧往往是 hallucination，这些帧不应参与梯度计算
> - 每条轨迹仅保留至首次成功（含）的 $T_i^{\text{valid}}$ 个有效步骤
>
> **Step 3 — Calculate Advantage → GRPO 更新**
>
> - 在一个 group 内（G 条轨迹），计算组相对 return：
>   $$\hat{A}^{(i)} = R(\tau^{(i)}) - \frac{1}{G}\sum_{j=1}^{G} R(\tau^{(j)})$$
>   即当前轨迹的 return 减去组内均值，正 advantage = 相对更好，负 advantage = 相对更差
> - 图右侧输出 $A_1^H, A_1^L, A_2^L, \ldots, A_G^L$（H = high / L = low advantage）
> - GRPO 目标（带 trajectory-length normalization）：
>   $$J_{\text{GRPO}}(\theta) = \mathbb{E}\left[\frac{1}{G}\sum_i \frac{1}{T_i^{\text{valid}}} \sum_{t=1}^{T_i^{\text{valid}}} \min\!\left(\rho_t^{(i)}\hat{A}^{(i)},\ \text{clip}(\rho_t^{(i)}, 1\pm\epsilon)\hat{A}^{(i)}\right)\right]$$
>   除以 $T_i^{\text{valid}}$ 的原因：KIR 产生的短 rollout（从关键帧出发）step 数少，不 normalize 会被长 rollout 淹没；normalize 后每步梯度贡献均等，让 task-critical segment 主导学习信号
>
> **整体逻辑串联**：KIR 缩短有效误差深度 → reward masking 过滤 post-success hallucination → group-relative advantage + length normalization 让 RL 主要从「关键交互区段」学习
